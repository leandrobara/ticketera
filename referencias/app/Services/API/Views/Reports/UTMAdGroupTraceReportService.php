...
class UTMAdGroupTraceReportService
{
    ...
    public function list(Client $client, array $options = [])
    {
        ...
        foreach ($acquisitionChannels as $acquisitionChannel) {
            ...
            $channelLeads = $leadsQuery
                ->select(['id', 'acquisition_channel_id', 'utm_campaign', 'utm_medium', 'quality'])
                ->get(['id', 'acquisition_channel_id', 'utm_campaign', 'utm_medium', 'quality'])
            ;
            ...
            foreach ($utmCampaignNames as $utmCampaignName) {
                ...
                foreach ($utmMediumNames as $utmMediumName) {
                    ...
                    $utmMediumLeadIds = $utmMediumLeads->pluck('id');
                    $utmMediumLeadsCount = $utmMediumLeadIds->unique()->count();

                    $utmMediumSales = $leadsSales->whereIn('lead_id', $utmMediumLeadIds);
                    $utmMediumProposals = $leadsProposals->whereIn('lead_id', $utmMediumLeadIds);

                    $utmMediumTotalSalesCount = $utmMediumSales->count();
                    $utmMediumTotalProposalsCount = $utmMediumProposals->count();

                    $utmMediumLeadsWithSalesCount = $utmMediumSales->pluck('lead_id')->unique()->count();
                    $utmMediumLeadsWithSentProposalsCount = $utmMediumProposals->pluck('lead_id')->unique()->count();

                    $qualityAvg = $utmMediumLeads->avg('quality');
                    $closeRateAvg = $utmMediumLeadsWithSentProposalsCount
                        ? ($utmMediumLeadsWithSalesCount * 100) / $utmMediumLeadsWithSentProposalsCount
                        : 0
                    ;
                    if ($closeRateAvg > 100) {
                        $closeRateAvg = 100;
                    ...
                    $reportRow = [
                        ...
                        'leads_count' => $utmMediumLeadsCount,
                        'total_sales_count' => $utmMediumTotalSalesCount,
                        'acquisition_channel_id' => $acquisitionChannelId,
                        'acquisition_channel_name' => $acquisitionChannelName,
                        'total_proposals_count' => $utmMediumTotalProposalsCount,
                        'leads_with_sales_count' => $utmMediumLeadsWithSalesCount,
                        'leads_with_proposals_count' => $utmMediumLeadsWithSentProposalsCount,
                    ];
