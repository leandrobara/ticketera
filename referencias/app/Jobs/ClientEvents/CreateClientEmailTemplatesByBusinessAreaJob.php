...
class CreateClientEmailTemplatesByBusinessAreaJob implements ShouldQueue
{
    ...
    {
        $clientyConfigEmailTpls = new Collection();
        $clientyConfigTplService = resolve(ClientyConfigEmailTemplateService::class);
        
        $clientyConfigEmailTplsForAll = $clientyConfigTplService->findForAllBusinessArea();
        $clientyConfigEmailTpls = $clientyConfigEmailTpls->merge($clientyConfigEmailTplsForAll);
        ...
        if ($this->leadsBusinessAreaName) {
            ...
            if ($businessArea) {
                ...
                $clientyConfigEmailTpls = $clientyConfigEmailTpls->merge($businessAreaTpls);
                ...
                if ($this->leadsBusinessAreaChildName) {
                    ...
                    if ($businessAreaChild) {
                        ...
                        $clientyConfigEmailTpls = $clientyConfigEmailTpls->merge($childTpls);
                    }
                }
            }
        }
        if ($clientyConfigEmailTpls->isEmpty()) {
            return;
        }

        $client = Client::findOrFail($this->clientId);
        $emailTplService = resolve(EmailTemplateService::class);
        $emailTpls = $emailTplService->createMultipleFromClientyConfigEmailTemplates(
            $clientyConfigEmailTpls, $client->users->first()
        );
    }
