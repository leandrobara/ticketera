...
class SendWAPINewTaskNotificationMessageJob implements ShouldQueue
{
    ...
    {
        $lockKey = 'SendWAPINewTaskNotificationMessageJob';
        ...
        try {
            ...
            $chatMessageString = view(
                'api.whatsapp-message-notification.task.new-task',
                compact('task', 'user', 'lead', 'taskUrl', 'leadUrl', 'assignerUser')
            )->render();
    ...
    protected function handleAndPersistErrorIfExists(TaskNotificationWhatsAppMessage $taskNotif): bool
    {
        ...
        if (!$taskNotif->dispatched_date) {
            $notifsService->persistFailReason($taskNotif, 'task_notification_has_not_dispatched_date');
            return true;
        }
        ...
        if (!$taskNotif->client->clientSettings->enable_new_task_whatsapp_message_alert) {
            $notifsService->persistFailReason($taskNotif, 'disabled_new_task_whatsapp_message_alert');
            return true;
        }
    ...
    {
        $taskNotif = TaskNotificationWhatsAppMessage::findOrFail($this->taskNotificationWhatsAppMessageId);
        resolve(WhatsAppNotificationEventsDispatcherService::class)->dispatchSendWAPINewTaskNotificationMessageJob(
            $taskNotif, $baseDelaySeconds
        );
