...
class MarkLeadContactEmailsAsBouncedJob implements ShouldQueue
{
    ...
    {
        $lockKey = 'MarkLeadContactEmailsAsBouncedJob:handle:' . implode(',', $this->leadContactEmailIds);
        ...
        $service->markAsBounced($leadContactEmails);

        if ($client->clientSettings->see_disabled_email_reason_as_label) {
            $this->assignInvalidEmailTagToLead($leadContactEmails);
        }
    }


    public function assignInvalidEmailTagToLead(Collection $leadContactEmails)
    {
        ...
        foreach ($leadContactEmails as $leadContactEmail) {
            ...
            $leadService->assignInvalidEmailTag($lead);
        }
    }
