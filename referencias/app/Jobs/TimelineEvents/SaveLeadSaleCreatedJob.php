...
class SaveLeadSaleCreatedJob implements ShouldQueue
{
    ...
    {
        $leadSale = LeadSale::findOrFail($this->leadSaleId);
        $user = $this->userId ? resolve(UserService::class)->findOrFail($this->userId) : null;
        resolve(EventsLogService::class)->saveLeadSaleCreated($user, $leadSale, $this->eventLogDate);
    }
