...
class ListClientRequest extends APIBaseRequest
{
    ...
    public function rules()
    {
        return [
            'page' => ['sometimes', 'int'],
            'limit' => ['sometimes', 'int'],
            'order' => ['sometimes', 'string', 'in:name'],
            'filters' => ['sometimes', 'array'],
            'filters.enabled' => ['sometimes', 'nullable', 'boolean'],
        ];
    }
    ...
    {
        $visibleFields = ['id', 'name', 'subdomain', 'timezone', 'created_at', 'enabled'];
        if (request()->input('withClientSettings')) {
            $visibleFields[] = 'clientSettings';
        }
