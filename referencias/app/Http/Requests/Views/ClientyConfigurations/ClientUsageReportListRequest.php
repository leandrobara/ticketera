...
class ClientUsageReportListRequest extends APIBaseRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'filters.client_id' => ['required', 'integer'],
            'filters.date_end' => ['required', 'nullable', 'date_format:Y-m-d\TH:i:sP'],
            'filters.date_start' => ['required', 'nullable', 'date_format:Y-m-d\TH:i:sP'],
        ];
    }
    ...
    public function withValidator($validator)
    {
        if (!$validator->failed()) {
            $validator->after(function ($validator) {
                ...
                $this->reportClient = Client::find(request()->input('filters')['client_id']);
                if (!$this->reportClient) {
                    $validator->errors()->add('client_id', 'client_does_not_exists');
                    return false;
                }
            });
        }
