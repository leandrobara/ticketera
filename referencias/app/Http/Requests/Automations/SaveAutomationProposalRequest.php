...
use App\Rules\InAutomationProposalReturnFields;


class SaveAutomationProposalRequest extends APIBaseRequest
{

    private $cancellingTags = null;
    private $cancellingStatus = null;
    private $sendEmailTemplate = null;
    private $interactionRuleAssignStatus = null;
    private $modifyLeadAfterSendRuleAssignStatus = null;


    public function rules()
    {
        return [
            // automation proposal resend rule
            'enabled' => ['required', 'boolean'],
            'resendRule.send_email_template_id' => ['sometimes', 'integer'],
            ...
            'resendRule.cancel_if_proposal_was_opened' => ['sometimes', 'boolean'],
            ...
            // automation proposal interaction
            'interactionRule.add_opened_proposal_tag' => ['sometimes', 'boolean'],
            'interactionRule.assign_status_id' => ['sometimes', 'integer', 'nullable'],
            'interactionRule.send_notification_email_to_user' => ['sometimes', 'boolean'],
            'interactionRule.notify_only_if_lead_quality_is_gt' => ['sometimes', 'nullable', 'integer'],
            // automation after send proposal
            ...
            'fields.*' => ['sometimes', new InAutomationProposalReturnFields()],
        ];
    }
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            if (!$validator->failed()) {
                ...
                $sendTemplateId = request()->input('resendRule.send_email_template_id');
                if ($sendTemplateId) {
                    $this->sendEmailTemplate = EmailTemplate::where([
                        'client_id' => $client->id, 'id' => $sendTemplateId
                    ])->first();

                    if (!$this->sendEmailTemplate) {
                        $validator->errors()->add('send_email_template_id', 'send_email_template_id_does_not_exists');
                    }
                }
                ...
                if ($statusId) {
                    ...
                    $this->modifyLeadAfterSendRuleAssignStatus = $status;
                }

                $statusId = request()->input('interactionRule.assign_status_id');
                if ($statusId) {
                    $status = Status::where('client_id', $client->id)->where('id', $statusId)->first();
                    if (!$status) {
                        $validator->errors()->add('assign_status_id', 'assign_status_does_not_exists');
                    ...
                    $this->interactionRuleAssignStatus = $status;
                }
            }
        });
    }
    ...
    public function validatedDTO(): AutomationProposalDTO
    {
        ...
        if ($val['resendRule'] ?? null) {
            ...
            $val['resendRule']['sendEmailTemplate'] = $this->sendEmailTemplate;
        ...
        if ($this->interactionRuleAssignStatus) {
            $val['interactionRule']['assignStatus'] = $this->interactionRuleAssignStatus;
        }
        if ($this->modifyLeadAfterSendRuleAssignStatus) {
            $val['modifyLeadAfterSendRule']['assignStatus'] = $this->modifyLeadAfterSendRuleAssignStatus;
        }
        $dto = AutomationProposalDTO::build($val);
        return $dto;
    }
