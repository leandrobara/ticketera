...
class CreateWAutomationSequenceRequest extends APIBaseRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            if (!$validator->failed()) {
                ...
                $wAutomations = WAutomationSequence::where([
                    'client_id' => $clientId, 'trigger_type' => 'after_tag_status_change'
                ])->get();
                ...
                if ($triggeringStatusIds) {
                    $triggeringStatus = Status::where('client_id', $clientId)->whereIn(
                    ...
                    if ($statusIsBeingUsed) {
                        $validator->errors()->add(
                            'triggering_tags_ids', 'triggering_status_ids_are_being_used_by_another_wautomation'
                        );
                        return false;
                    }
                ...
                if ($cancellingTagIds) {
                    $cancellingTagIds = Tag::where('client_id', $clientId)->whereIn('id', $cancellingTagIds)->get();
                    if ($cancellingTagIds->count() != count($cancellingTagIds)) {
                        $validator->errors()->add('cancelling_tags_ids', 'not_all_cancelling_tags_exist');
                        return false;
                    }
                    $this->cancellingTags = $cancellingTagIds;
                }
