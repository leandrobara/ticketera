...
class InjectClientZapier
{
    ...
    public function handle($req, Closure $next)
    {
        try {
            ...
            if (!$subdomain) {
                throw new Exception('InjectClientZapier | Client subdomain parameter is missing');
            }
            $cleanSubdomain = $this->getCleanSubdomain($subdomain);

            $client = resolve(ClientRepository::class)->findOneBySubdomain($cleanSubdomain);
            if (!$client) {
                throw new Exception('InjectClientZapier | Client does not exist');
            }
