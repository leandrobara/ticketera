...
class WAutomationWAPSenderWorkerController extends BaseAPIController
{
    ...
    public function retryWAPSenderWAutomationsFailedSendings(Request $req)
    {
        $lockKey = 'retryWAPSenderWAutomationsFailedSendings';
        ...
        foreach ($clients as $clientIndex => $client) {
            ...
            foreach ($users as $user) {
                try {
                    ...
                    $wAutomationLogs = $wAutomationLogService->findFailedWAPSenderJobsEnabledToRetry(
                        $user, self::ERRORS_ENABLED_TO_RETRY
                    );
                    if ($wAutomationLogs->isEmpty()) {
                        $this->printAndLogInfo("[NO MESSAGES TO RETRY] - EMPTY wAutomationLogs", ['isError' => true]);
                        continue;
                    }
                    ...
                    foreach ($wAutomationLogs as $wAutomationLog) {
                        $wapSendingMsgToRetry = $wAutomationLog->sentWhatsAppSendingMessage;
                        $errorMsg = $this->getInvalidErrorMessage($wapSendingMsgToRetry, $user);
                        if ($errorMsg) {
                            $this->printAndLogNotSuccessDispatch($wAutomationLog, $wapSendingMsgToRetry, $errorMsg);
                            continue;
                        }

                        $delaySecs = ($delaySecs ?? 0) + 8;
                        $whatsAppSendingMessageService->markAsDispatchedToRetry($wapSendingMsgToRetry);
                        $whatsAppEventsDispatcherService->dispatchSendWAutomationWAPSenderMessageJob(
                            $wapSendingMsgToRetry, $delaySecs
                        );

                        $this->printAndLogSuccessDispatch($wAutomationLog, $wapSendingMsgToRetry, $delaySecs);
                        $this->printSeparator();
                    }
    ...
    public function getInvalidErrorMessage(
        WhatsAppSendingMessage $wapSendingMsgToRetry,
        User $user
    ): ?string {
        ...
        // No! Por que muchos se buscan con LIKE %xxx%
        // if (!in_array($wapSendingMsgToRetry->error_message, self::ERRORS_ENABLED_TO_RETRY)) {
        //     return 'wap_sending_message_error_message_is_not_enable_to_retry';
        // }
    ...
    private function printAndLogNotSuccessDispatch(
        WAutomationLog $wAutomationLog,
        WhatsAppSendingMessage $wapSendingMsgToRetry,
        string $errorMsg
    ) {
        $msg = "wAutomationLog ID: {$wAutomationLog->id} | ";
    ...
    private function printAndLogSuccessDispatch(
        WAutomationLog $wAutomationLog,
        WhatsAppSendingMessage $wapSendingMsgToRetry,
        int $delaySecs
    ): void {
        $msg = "wAutomationLog ID: {$wAutomationLog->id} | wapSendingMsgToRetry ID: {$wapSendingMsgToRetry->id}";
    ...
    {
        return Log::channel('WAutomationWAPSenderWorkerControllerInfo');
    }
