...
class CreateShowCreditRequest extends FormRequest
{
    public function rules(): array
    {
        return [
            ...
            'section' => ['required', Rule::in(['cast', 'technical'])],
            'sort_order' => ['nullable', 'integer', 'min:1'],
            'character_name' => ['nullable', 'string', 'max:160'],
            'role_label' => ['required', 'string', 'max:160'],
    ...
    public function withValidator($validator): void
    {
        if (!$validator->failed()) {
            $validator->after(function ($validator) {
                if (!filled($this->input('person_id')) && !filled($this->input('display_name_override'))) {
